	...
	<section>
		...
		<el-table :data="mddList" border style="width: 100%; margin-top: 20px;" v-loading="loading.list">
			...
			<el-table-column prop="loc.label" label="所在地区" width="150" show-overflow-tooltip>
				<template scope="scope">
					<span v-if="scope.row.loc.parent">{{ scope.row.loc.parent.label }} / </span> {{ scope.row.loc.label }}
				</template>
			</el-table-column>
		...
		<el-pagination class="fenye" :current-page.sync="pagination.current_page" :page-size="pagination.per_page" :total="pagination.total" layout="total, prev, pager, next">
		</el-pagination>
		...
		<el-dialog :title="mop.current.id ? '修改' : '创建'" v-model="mop.show" :close-on-click-modal="false" size="tiny">
			<el-form label-position="top" :model="mop.current" ref="mdd">
				<el-form-item label="选择所在地" prop="cascader" :rules="[{ required: true, message: '选择所在地'}]">
					<el-cascader placeholder="可搜索" :options="locList" v-model="mop.current.cascader" :props="props" filterable change-on-select></el-cascader>
				</el-form-item>
				<el-form-item label="目的地" prop="title" :rules="[{ required: true, message: '目的地不能为空'}, {min: 2, max: 50, message: '长度在2-50字'}]">
...
<script>
	...
	import ImageSelect from '@/views/picture/ImageSelect'
